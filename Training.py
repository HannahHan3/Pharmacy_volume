...
path='/Users/huiminhan/Dropbox/UMD/CHIDS/chidsdata'
os.chdir(path)
...
def read_xls(sheet,weekend=True):
    ...
    df['first_10d']=pd.get_dummies(df['first_10d'])[1]
    df['last_10d']=pd.get_dummies(df['last_10d'])[1]
...
cols = [
     ...
     'Bernardo Center','Carlsbad','Escondido','Oceanside MOB','Total',
...
def lr_model(df_dict,filename):
    ...
    for i in range(36):
        ...
        scaler = MinMaxScaler()
        scaler.fit(split_data(df)[0])
        lr=LinearRegression(normalize=False)
        lr.fit(scaler.transform(split_data(df)[0]),split_data(df)[1])
        lr_ypred=lr.predict(scaler.transform(split_data(df)[4]))
...
def lasso_model(df_dict,filename):
    ...
    for i in range(36):
        ...
        for alpha in np.logspace(-15,7,100):
            ...
            lasso.fit(scaler.transform(split_data(df)[0]),split_data(df)[1])
            lasso_ypred=lasso.predict(scaler.transform(split_data(df)[2]))
        ...
        lasso_best.fit(scaler.transform(split_data(df)[0]),split_data(df)[1])
        lasso_ypred=lasso_best.predict(scaler.transform(split_data(df)[4]))
...
def ridge_model(df_dict,filename):
    ...
    for i in range(36):
        ...
        for alpha in np.logspace(-15,7,100):
            ...
            ridge.fit(scaler.transform(split_data(df)[0]),split_data(df)[1])
            ridge_ypred=ridge.predict(scaler.transform(split_data(df)[2]))
            ridge_mae=MAE(split_data(df)[3],ridge_ypred)
            mae_gs[alpha]=ridge_mae
        alpha_best=min(mae_gs,key=mae_gs.get)
        alpha_ridge[i]=alpha_best
        ridge_best=Ridge(alpha=alpha_best)
        ridge_best.fit(scaler.transform(split_data(df)[0]),split_data(df)[1])
        ridge_ypred=ridge_best.predict(scaler.transform(split_data(df)[4]))
...
def sgd_model(df_dict,filename):
    ...
    for i in range(36):
        ...
        for loss in ['squared_loss','huber','epsilon_insensitive']:
            for alpha in np.logspace(-15,7,23):
                sgd=SGDRegressor(loss=loss,
                                 alpha=alpha)
                sgd.fit(scaler.transform(split_data(df)[0]),split_data(df)[1])
                sgd_ypred=sgd.predict(scaler.transform(split_data(df)[2]))
                sgd_mae=MAE(split_data(df)[3],sgd_ypred)
                mae_gs=mae_gs.append(pd.DataFrame({'loss':[loss],
                                                   'alpha':[alpha],
                                                   'valid_mae':[sgd_mae]}),ignore_index=True)
        parameter_best=mae_gs.loc[mae_gs['valid_mae'].idxmin(),:]
        sgd_best=SGDRegressor(loss=parameter_best['loss'],
                              alpha=parameter_best['alpha'])
        sgd_best.fit(scaler.transform(split_data(df)[0]),split_data(df)[1])
        sgd_ypred=sgd_best.predict(scaler.transform(split_data(df)[4]))
